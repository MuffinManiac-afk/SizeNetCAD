...
def truncate_new_dataset_axis(root_path,save_path,div_num,partial=False):
    ...
    for folder in tqdm(folder_list):
        ...
        for img_list in image_dict.values():
            ...
            mean_ratio = total_percent_sum / len(img_list)
        ...
        for axis, img_list in image_dict.items():
            ...
            mean_ratio = total_percent_sum / len(img_list)
            mean_index = percent_list.index(min([k for k in percent_list if k >= mean_ratio]))

            if mean_ratio >= total_mean:
                stop_index = mean_index
            elif mean_ratio < total_mean:
                stop_index = mean_index * (1/5)
